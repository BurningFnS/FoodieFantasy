...
public class PlatformDestory : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other) //when the platform colliders with the trigger...
    {
        ...
        myPlat = (GameObject)Instantiate(Platform_Blue, new Vector2(Random.Range(26.0f, 43.5f), player.transform.position.y + (2 + Random.Range(-0.5f, 0.5f))), Quaternion.identity);
